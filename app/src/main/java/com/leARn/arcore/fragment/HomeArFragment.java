...
import com.leARn.arcore.Adapters.AssetAdapter;
import com.leARn.arcore.AsyncFileDownloader;
import com.leARn.arcore.AsyncHttpRequest;
import com.leARn.arcore.ItemClick;
import com.leARn.arcore.PolyApi;
import com.leARn.arcore.PolyApi_SERVICE;
import com.leARn.arcore.R;
import com.leARn.arcore.bean.Asset;
import com.leARn.arcore.bean.ModelList;
import com.leARn.common.helpers.DisplayRotationHelper;
import com.leARn.common.helpers.TapHelper;
import com.leARn.common.rendering.ObjectRenderer;
...
import com.leARn.common.helpers.CameraPermissionHelper;
import com.leARn.common.helpers.SnackbarHelper;
import com.leARn.common.rendering.BackgroundRenderer;
import com.leARn.common.rendering.PlaneRenderer;
import com.leARn.common.rendering.PointCloudRenderer;
 ...
 */
public class HomeArFragment extends Fragment implements GLSurfaceView.Renderer, ItemClick {
  ...
  private String BASE_URL = "https://poly.googleapis.com/v1/";
  PolyApi_SERVICE polyApi;
  ...
  private void updateModelList(String query) {
    ...
    Retrofit retrofit = new Retrofit.Builder()
            ...
            .build();
    polyApi = (PolyApi_SERVICE) retrofit.create(PolyApi_SERVICE.class);

    Call<ModelList> models = polyApi.getModel(query, "OBJ", PolyApi.API_KEY);
  ...
  public void onResume() {
    ...
    if (session == null) {
      ...
      try {
        ...
        if (!CameraPermissionHelper.hasCameraPermission((AppCompatActivity) getActivity())) {
          CameraPermissionHelper.requestCameraPermission((AppCompatActivity) getActivity());
          return;
        }
      ...
      if (message != null) {
        messageSnackbarHelper.showError((AppCompatActivity) getActivity(), message);
    ...
    try {
      ...
      messageSnackbarHelper.showError((AppCompatActivity) getActivity(), "Camera not available. Please restart the app.");
      session = null;
      return;
    }
    ...
    messageSnackbarHelper.showMessage((AppCompatActivity) getActivity(), "Searching for surfaces...");
  }
  ...
  public void onPause() {
    ...
    if (session != null) {
      ...
      surfaceView.onPause();
      session.pause();
      messageSnackbarHelper.hide((AppCompatActivity) getActivity());
    }
  }
  ...
  public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] results) {
    if (!CameraPermissionHelper.hasCameraPermission((AppCompatActivity) getActivity())) {
      Toast.makeText(getContext(), "Camera permission is needed to run this application", Toast.LENGTH_LONG)
          .show();
      if (!CameraPermissionHelper.shouldShowRequestPermissionRationale((AppCompatActivity) getActivity())) {
        // Permission denied with checking "Do not ask again".
        CameraPermissionHelper.launchPermissionSettings((AppCompatActivity) getActivity());
      }
      getActivity().finish();
    }
  }
...
//    FullScreenHelper.setFullScreenOnWindowFocusChanged((AppCompatActivity) getActivity(), hasFocus);
//  }
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    try {
      ...
      pointCloudRenderer.createOnGlThread(/*context=*/ getContext());
    } catch (IOException e) {
  ...
  private void importDownloadedObject() {
    try {
      ...
      if (objBytes == null || textureBytes == null) {
        ...
        //Toast.makeText(getContext(), "Downloaded asset doesn't have OBJ data and a PNG texture", Toast.LENGTH_SHORT).show();
        return;
      }
      Log.d(TAG, "Importing OBJ.");
     // Toast.makeText(getContext(), "Importing OBJ", Toast.LENGTH_SHORT).show();

      virtualObject.createOnGlThread(/*context=*/getContext(), objBytes, textureBytes);
  ...
  private void showAttributionToast() {
    ...
    getActivity().runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        Toast.makeText(getContext(), mAttributionText, Toast.LENGTH_LONG).show();
      }
    });
  }
  ...
  public void ItemClick(int position) {
    ...
    asset_id = asset.getId().split("/")[1].trim();
    Log.e("Asset" , asset.getId() + " " + asset_id);
